...
{ 
    ...
    public struct ScalarField<T> : IDisposable where T : struct
    {
        ...
        public T GetData(int3 xyz)
        {
            return _data[IndexUtilities.XyzToIndex(xyz, Width, Depth)];
        }
        ...
        public int3 GetXYZIndex(int index)
        { 
            int x = IndexUtilities.XyzToX(index, Width);
            int y = IndexUtilities.XyzToY(index, Width, Depth);
            int z = IndexUtilities.XyzToZ(index, Width, Depth);
